...
def augpath(path, suffix='', prefix='', ext=None, base=None):
    ...
    CommandLine:
        python -m pypdir.util_path augpath
    ...
    Example:
        ...
        >>> prefix = 'pref_'
        >>> ext = '.baz'
        >>> newpath = pydir.augpath(path, suffix, prefix, ext=ext, base='bar')
...
#     Example:
#         >>> from pypdir.util_platform import *
...
def truepath(path, real=False):
    ...
    CommandLine:
        python -m pypdir.util_path truepath

    Example:
        >>> import pypdir
        >>> assert pydir.truepath('~/foo') == join(pydir.userhome(), 'foo')
        >>> assert pydir.truepath('~/foo') == pydir.truepath('~/foo/bar/..')
        >>> assert pydir.truepath('~/foo', real=True) == pydir.truepath('~/foo')
    """
...
def ensuredir(dpath, mode=0o1777, verbose=None):
    ...
    Example:
        >>> from pypdir.util_platform import *  # NOQA
        >>> import pypdir
        >>> cache_dpath = pydir.ensure_app_cache_dir('pypdir')
        >>> dpath = join(cache_dpath, 'ensuredir')
        >>> if exists(dpath):
        ...     os.rmdir(dpath)
        >>> assert not exists(dpath)
        >>> pydir.ensuredir(dpath)
        >>> assert exists(dpath)
        >>> os.rmdir(dpath)
    """
...
if __name__ == '__main__':
    r"""
    CommandLine:
        python -m pypdir.util_path
        python -m pypdir.util_path all
    """
